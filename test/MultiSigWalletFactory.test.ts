...
describe("Contract 'MultisigWalletFactory'", () => {
  ...
  describe("Function 'deployNewWallet()'", () => {
    it("Creates new wallet instance with selected parameters", async () => {
      ...
      await expect(await factory.deployNewWallet(ownerAddresses, REQUIRED_APPROVALS)).to.emit(
        factory,
        EVENT_NAME_NEW_WALLET_DEPLOYED_BY_FACTORY
      );
    ...
    it("Is reverted if the input owner array is empty", async () => {
      ...
      await expect(factory.deployNewWallet([], REQUIRED_APPROVALS)).to.be.revertedWithCustomError(
        walletFactory,
    ...
    it("Is reverted if the input number of required approvals is zero", async () => {
      ...
      await expect(factory.deployNewWallet(ownerAddresses, requiredApprovals)).to.be.revertedWithCustomError(
        walletFactory,
    ...
    it("Is reverted if one of the input owners is the zero address", async () => {
      ...
      await expect(factory.deployNewWallet(ownerAddressArray, requiredApprovals)).to.be.revertedWithCustomError(
        walletFactory,
